...
`SELinux` が有効な環境では、HTTPサーバーに配置したファイルにも適切な `SELinux` の「コンテキスト」が必要です。
`SELinux` の「コンテキスト(ユーザー:ロール:タイプ)」の「タイプ」を `httpd_sys_content_t`になっていれば、OKです
...
なっていない場合は、以下のコマンドセットで「コンテキスト(ユーザー:ロール:タイプ)」の「タイプ」を `httpd_sys_content_t` に変更してあげます。
...
HTTPサーバー上に配置されている、`/usr/share/nginx/html/pxeboot.ipxe` を編集して、`CoreOS`のイメージファイル(rootfs, initramfs,kernel)を、`iPXE`のブートメニューに紐付けて埋め込んでいきます。
...
kernel http://bastion.example.localdomain/rhcos-4.6.8-x86_64-live-kernel-x86_64 ip=dhcp rd.neednet=1 console=tty0 console=ttyS0 coreos.inst=yes coreos.inst.install_dev=sda initrd=rhcos-4.6.8-x86_64-live-initramfs.x86_64.img  coreos.live.rootfs_url=http://bastion.example.localdomain/rhcos-4.6.8-x86_64-live-rootfs.x86_64.img coreos.inst.ignition_url=http://bastion.example.localdomain/bootstrap.ign
initrd http://bastion.example.localdomain/rhcos-4.6.8-x86_64-live-initramfs.x86_64.img
boot
...
この例で`bastion.example.localdomain`は、`踏み台サーバー`(`HTTPサーバー`/`TFTPサーバー`が同居)の`FQDN`です。
