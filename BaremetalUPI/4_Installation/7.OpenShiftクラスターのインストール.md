...
※この絵では、Infra Node も絵に含まれていますが、Infora Node は、Master Node と Worker Node のインストールを完了させた後に追加する事にします。技術的な制約ではな無く、初回インストール時に一気に Node を作成するとインストールが失敗した時のリカバリーが大変なため、そのような手順にしています。
