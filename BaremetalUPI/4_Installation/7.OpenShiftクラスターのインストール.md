...
この要領で、とりあえず合計7本 (`Bootstrap` x 1、`Master` x 3、`Worker` x 3) の `RHEL CoreOS`ノードを準備します。
...
※Infraノードが絵に描かれていますが、ここではまだ導入しません。技術的な理由ではなく、慣れてない時に一度にたくさんのノードを作成すると、失敗した時にリカバリーが大変なためです。
...
黒い画面にログがつらつらと流れている状態で、次のステップに進んで大丈夫です。
...
INFO Waiting up to 20m0s for the Kubernetes API at https://api.ocp46.example.localdomain:6443...
INFO API v1.19.0+7070803 up
INFO Waiting up to 30m0s for bootstrapping to complete...
DEBUG Bootstrap status: complete
INFO It is now safe to remove the bootstrap resources
DEBUG Time elapsed per stage:
DEBUG Bootstrap Complete: 43s
INFO Time elapsed: 43s
...
この例ではすぐにコマンドが終わっていますが、これはあくまで`BootSrap` のシークエンスの完了を確認するためのコマンドで、コマンドの実行タイミングが、`BootStrapノード`の起動直後だったりすると、20～30分かかる事もあります。
...
csr-2r22s   17s   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Pending
csr-9rqbr   13m   kubernetes.io/kubelet-serving                 system:node:m3.ocp46.example.localdomain                                    Approved,Issued
csr-9sm6l   13m   kubernetes.io/kubelet-serving                 system:node:m2.ocp46.example.localdomain                                    Approved,Issued
csr-bvs74   29s   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Pending
csr-fcrwk   12m   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-htxlk   9s    kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Pending
csr-krmvv   13m   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-phw84   13m   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-z9j62   12m   kubernetes.io/kubelet-serving                 system:node:m1.ocp46.example.localdomain                                    Approved,Issued
[root@bastion openshift]#
```
...
```
[root@bastion openshift]# oc adm certificate approve csr-2r22s
certificatesigningrequest.certificates.k8s.io/csr-2r22s approved
[root@bastion openshift]#
```
...
もし`Wroker Node`が 3本ある場合は、6回 `CSR`のapproveが必要になるはずです。
以下のケースでは、1サイクル目の csr (`Worker Node`3本で3つ)の approve が完了し、次の `Pending` x 3 が出てきた状態です。

```
[root@bastion openshift]# oc get csr
NAME        AGE     SIGNERNAME                                    REQUESTOR                                                                   CONDITION
csr-2r22s   4m26s   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-4pcns   3m40s   kubernetes.io/kubelet-serving                 system:node:w3.ocp46.example.localdomain                                    Pending
csr-6gbjh   3m33s   kubernetes.io/kubelet-serving                 system:node:w1.ocp46.example.localdomain                                    Pending
csr-9rqbr   17m     kubernetes.io/kubelet-serving                 system:node:m3.ocp46.example.localdomain                                    Approved,Issued
csr-9sm6l   17m     kubernetes.io/kubelet-serving                 system:node:m2.ocp46.example.localdomain                                    Approved,Issued
csr-bvs74   4m38s   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-fcrwk   16m     kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-htxlk   4m18s   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-krmvv   18m     kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-phw84   18m     kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-ppcmc   3m32s   kubernetes.io/kubelet-serving                 system:node:w2.ocp46.example.localdomain                                    Pending
csr-z9j62   16m     kubernetes.io/kubelet-serving                 system:node:m1.ocp46.example.localdomain                                    Approved,Issued
[root@bastion openshift]# 
```

大変なので、ワンライナーのコマンドで、3枚一気に approveしてしまいます。

```
[root@bastion openshift]# oc get csr -o go-template='{{range .items}}{{if not .status}}{{.metadata.name}}{{"\n"}}{{end}}{{end}}' | xargs oc adm certificate approve
certificatesigningrequest.certificates.k8s.io/csr-4pcns approved
certificatesigningrequest.certificates.k8s.io/csr-6gbjh approved
certificatesigningrequest.certificates.k8s.io/csr-ppcmc approved
[root@bastion openshift]# 
```

approve が全て完了すると、以下のようになるはずです。

```
[root@bastion openshift]# oc get csr
NAME        AGE     SIGNERNAME                                    REQUESTOR                                                                   CONDITION
csr-2r22s   7m8s    kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-4pcns   6m22s   kubernetes.io/kubelet-serving                 system:node:w3.ocp46.example.localdomain                                    Approved,Issued
csr-6gbjh   6m15s   kubernetes.io/kubelet-serving                 system:node:w1.ocp46.example.localdomain                                    Approved,Issued
csr-9rqbr   20m     kubernetes.io/kubelet-serving                 system:node:m3.ocp46.example.localdomain                                    Approved,Issued
csr-9sm6l   20m     kubernetes.io/kubelet-serving                 system:node:m2.ocp46.example.localdomain                                    Approved,Issued
csr-bvs74   7m20s   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-fcrwk   19m     kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-htxlk   7m      kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-krmvv   20m     kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-phw84   20m     kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:openshift-machine-config-operator:node-bootstrapper   Approved,Issued
csr-ppcmc   6m14s   kubernetes.io/kubelet-serving                 system:node:w2.ocp46.example.localdomain                                    Approved,Issued
csr-z9j62   19m     kubernetes.io/kubelet-serving                 system:node:m1.ocp46.example.localdomain                                    Approved,Issued
[root@bastion openshift]#
```

通常であれば、CSR を approve した直後に、Worker ノードが `oc get nodes` で表示されるようになっているはずです。
...
m1.ocp46.example.localdomain   Ready    master   21m     v1.19.0+7070803
m2.ocp46.example.localdomain   Ready    master   22m     v1.19.0+7070803
m3.ocp46.example.localdomain   Ready    master   22m     v1.19.0+7070803
w1.ocp46.example.localdomain   Ready    worker   8m24s   v1.19.0+7070803
w2.ocp46.example.localdomain   Ready    worker   8m23s   v1.19.0+7070803
w3.ocp46.example.localdomain   Ready    worker   8m31s   v1.19.0+7070803
[root@bastion openshift]#
```
...
INFO Waiting up to 40m0s for the cluster at https://api.ocp46.example.localdomain:6443 to initialize...
...
INFO Access the OpenShift web-console here: https://console-openshift-console.apps.ocp46.example.localdomain
INFO Login to the console with user: "kubeadmin", and password: "NqrXg-pdjy9-2c2er-MYTzr"
INFO Time elapsed: 49s
[root@bastion openshift]#
```

コマンドを実行した後は待つだけです。上記の例では、前の作業から時間を空けてから実行したせいで 49秒ほどで完了しています。

このコマンドもあくまで、インストールのプロセスの完了を確認するためのコマンドです。
